	...
	methods: {
		...
		_initSeller() {
			axios.get('static/data.json?id=' + this.seller.id).then((res) => {
                ...
                this.seller0 = Object.assign({}, this.seller0, res.data.seller);
				this.$nextTick(() => {
		...
		_initPicScroll() {
	    	...
	    	const PIC_WIDTH = 120;
	    	const MARGIN = 6;
	    	let picLen = this.seller0.pics.length;
	    ...
	    toggleFavorite() {
            ...
            saveToLocal(this.seller0.id, 'favorite', this.favorite);
	    }
	},
	components: {
		star,
